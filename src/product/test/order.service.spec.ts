...
describe("OrderService", () => {
  ...
  describe("createOrder", () => {
    it("should throw an error if cart is empty", async () => {
      ...
      // Act & Assert
      await expect(service.createOrder(userId, orderDto)).rejects.toThrow(BadRequestException)
    ...
    it("should create an order successfully", async () => {
      ...
      const orderDto = {
        ...
        paymentDetails: {
          amount: 300, // Sum of product prices: 100 + 200 = 300
          paymentMethod: PaymentMethod.CREDIT_CARD,
          cardNumber: "4111111111111111",
          cardExpiry: "12/24",
          cardCVC: "123",
      ...
      mockUserModel.findById.mockResolvedValue({
        _id: userId,
        cart: [
          { productId: product1Id, quantity: 1, addedAt: new Date() },
          { productId: product2Id, quantity: 2, addedAt: new Date() },
        ],
      })
      ...
      const savedOrder = {
        ...
        userId,
        products: [
          { productId: product1Id, quantity: 1, price: 100, productName: "Product 1" },
          { productId: product2Id, quantity: 2, price: 100, productName: "Product 2" },
        ],
        totalAmount: 300,
        status: "processing",
        toObject: () => ({
      ...
      expect(paymentService.processPayment).toHaveBeenCalledWith(orderDto.paymentDetails)
      expect(mockUserModel.findByIdAndUpdate).toHaveBeenCalledWith(userId, { $set: { cart: [] } }, { new: true })
    })
  })
})
